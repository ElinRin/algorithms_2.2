...
{
    cout<<a.brr.size()<<endl;
    int i;
    for (i=0;i<a.brr.size();++i)
    {
        cout<<a.brr[i]<<endl;
    }
    return cout;
...
double S (polygon &a)
{
     int i;
     double S1=0;
     for (i=1;i<a.brr.size()-1;++i)
     {
         S1=S1+S_triangle(a.brr[i],a.brr[i+1],a.brr[0]);
     }
     return S1;
}
...
bool pr_point_krug(circle c,point a)
{
...
{
    vector<point> arr;
    if (pr_point_krug(c,a))
       return arr;
    double u=acos(c.r/sqrt(RR(a,c.a))),r=sqrt(RR(a,c.a)-c.r*c.r);
    point z;
    z=povorot_vector(a-c.a,u);
    z.x=(z.x/sqrt(RR(a,c.a)))*r;
    z.y=(z.y/sqrt(RR(a,c.a)))*r;
    arr.push_back(c.a+z);
...
double Dlina_dugi(circle a,point q,point w)
{
     double u=atan2((vc(q-a.a,w-a.a)),ck(q-a.a,w-a.a));
     return abs(u)*a.r;
}
int main()
{
